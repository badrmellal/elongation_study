"""EXPOSURE CONTROL (exposure/PROTOCOL.md): same recipe as unseen_voice/ws/train.py, but the
training set adds the paper reciters. --variant M1 keeps their rows EXCEPT evaluated-verse texts
(voice exposed, recordings unseen); --variant M2 adds those too (recordings exposed).
...
from common import PREP, PROC, RUNS, ROOT, EXCLUDED, norm_ar, lev

HELD3 = {"sahl_yassin", "akram_alalaqimy", "muhsin_al_qasim"}

ap = argparse.ArgumentParser()
ap.add_argument("--variant", required=True, choices=["M1", "M2"])
...
ap.add_argument("--order-seed", type=int, default=None, help="Amendment 4 F1: changes batch order only")
a = ap.parse_args()
a.run = a.run or f"wb_{a.variant}"
...
ev = json.load(open(ROOT / "eval_texts.json", encoding="utf-8"))["eval_texts"]
ev = {k: set(v) for k, v in ev.items()}
train, val, arrays, dropped_eval = [], [], {}, 0
for j in sorted(list(PREP.glob("*.json")) + list((ROOT / "prep_paper").glob("*.json"))):
    idx = json.load(open(j, encoding="utf-8"))
    key = str(j.parent.name) + "/" + j.stem
    arrays[key] = np.load(j.with_suffix(".npy"), mmap_mode="r")
    split = "train" if idx["source"].startswith("train") else "val"
    for off, n, text, rec in idx["rows"]:
        assert rec not in HELD3
        if a.variant == "M1" and rec in ev and norm_ar(text) in ev[rec]:
            dropped_eval += 1
            continue
        ids = tok(text).input_ids
        if len(ids) > 448:
            continue
        (train if split == "train" else val).append((key, off, n, ids, text, rec))
val_sub = random.Random(a.seed).sample(val, min(a.val_n, len(val)))
emit(event="data", variant=a.variant, dropped_eval_rows=dropped_eval, train=len(train), val=len(val), val_sub=len(val_sub),
...
def batches():
    rng = random.Random((a.order_seed if a.order_seed is not None else a.seed) + 1); order = []
    while True:
        if not order:
