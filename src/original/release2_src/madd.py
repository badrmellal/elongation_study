...
Why the segmentation is text-driven rather than acoustic:
a voicing+flux detector finds voiced RUNS spanning several phonemes (median
1735 ms), not held vowels, and natural mujawwad is MORE dynamic than murattal,
so held vowels are not separable by voicing continuity alone. Madd, however, is
ORTHOGRAPHICALLY MARKED: it does not have to be found acoustically, the text says
where it must be and only its time boundaries need to be recovered.

The rabah2026 wav2vec2 vocab carries the diacritic set used by this text,
including dagger alif (U+0670), so the diacritized text aligns directly.

Produces madd occurrences with tajwid class and expected length in harakat counts,
and, through madd_intervals, their start/end times in seconds.
"""
...
def find_madd(text: str) -> List[Dict]:
    ...
    for i, ch in enumerate(text):
        ...
        # NOTE: when the madd letter is word-final, nxt2 is the first letter of
        # the next word, so a following-word hamza matches here and is labelled
        # muttasil; the munfasil branch below is consequently never reached on
        # this text. Both are 4-5 harakat classes, so the class-length ordering
        # reported from this locator is unaffected, but the label does not
        # distinguish same-word from next-word hamza. Left as-is because
        # changing it would relabel madd and invalidate the committed results.
